...
(function () {
  ...
  function boot() {
    ...
    var RENDERERS = {
      ...
      article: function (p) {
        ...
        var body = (p.sections || []).map(function (s) {
          var blocks = (s.blocks || []).map(function (b) {
            ...
            if (b.type === "note") return '<aside class="callout" data-item><span class="material-symbols-rounded callout__icon" aria-hidden="true">lightbulb</span><span>' + esc(t(b.text)) + "</span></aside>";
            if (b.type === "links") {
              var lis = (b.items || []).map(function (li) {
                return '<li><a class="src-link" href="' + esc(li.url) + '" target="_blank" rel="noopener">' +
                  '<span class="material-symbols-rounded" aria-hidden="true">north_east</span>' + esc(t(li.label)) + "</a></li>";
              }).join("");
              return '<ul class="src-links">' + lis + "</ul>";
            }
            if (b.type === "ul") {
      ...
      /* ---- practice: tabbed glossary + flashcards + quiz (session-only) ---- */
      practice: function (p) {
        var lang = L.state.lang;
        var L1 = lang === "en" ? "Glossary" : "詞彙表";
        var L2 = lang === "en" ? "Flashcards" : "翻卡";
        var L3 = lang === "en" ? "Quiz" : "測驗";
        return head(p) +
          '<div class="seg" role="tablist" id="practiceSeg">' +
            '<button class="seg__btn seg__btn--active" type="button" role="tab" data-tab="glossary">' +
              '<span class="material-symbols-rounded" aria-hidden="true">menu_book</span>' + esc(L1) + "</button>" +
            '<button class="seg__btn" type="button" role="tab" data-tab="flashcards">' +
              '<span class="material-symbols-rounded" aria-hidden="true">style</span>' + esc(L2) + "</button>" +
            '<button class="seg__btn" type="button" role="tab" data-tab="quiz">' +
              '<span class="material-symbols-rounded" aria-hidden="true">quiz</span>' + esc(L3) + "</button>" +
          "</div>" +
    ...
    var WIRE = {
      ...
      practice: function (p) {
        ...
        var segBtns = [].slice.call(pageEl.querySelectorAll("#practiceSeg .seg__btn"));
        var lang = L.state.lang;

        /* ---------- glossary: searchable definition list ---------- */
        function glossaryView() {
          var ph = lang === "en" ? "Search terms…" : "搜尋術語…";
          view.innerHTML =
            '<div class="toolbar"><input id="glossSearch" class="search" type="search" autocomplete="off" placeholder="' +
              esc(ph) + '" aria-label="' + esc(ph) + '" /></div>' +
          ...
          function paint(q) {
            ...
            }).join("") : '<p class="empty">' + (lang === "en" ? "No matches." : "查無結果。") + "</p>";
          }
          if (search) search.addEventListener("input", function () { paint(this.value.trim().toLowerCase()); });
          paint("");
        }

        /* ---------- flashcards: click to flip (CSS 3D) ---------- */
        function flashView() {
          var hint = lang === "en" ? "Tap a card to flip it." : "點卡片即可翻面。";
          view.innerHTML = '<p class="practice-hint">' + esc(hint) + "</p>" +
            '<div class="flash-grid">' + (p.flashcards || []).map(function (c, i) {
              return '<button class="flip-card" type="button" data-item data-i="' + i + '" aria-pressed="false">' +
                '<span class="flip-card__inner">' +
                  '<span class="flip-face flip-face--front">' + esc(t(c.front)) + "</span>" +
                  '<span class="flip-face flip-face--back">' + esc(t(c.back)) + "</span>" +
                "</span></button>";
            }).join("") + "</div>";
          [].forEach.call(view.querySelectorAll(".flip-card"), function (card) {
        ...
        /* ---------- quiz: multiple choice + instant feedback + score ---------- */
        function quizView() {
          var qs = p.quiz || [];
          var answered = 0, correct = 0;
          var scoreLabel = lang === "en" ? "Score" : "得分";
          view.innerHTML =
            '<div class="quiz-score" id="quizScore" aria-live="polite">' + esc(scoreLabel) + ": 0 / " + qs.length + "</div>" +
            '<ol class="quiz-list">' + qs.map(function (q, qi) {
              var opts = (q.options || []).map(function (o, oi) {
                return '<button class="quiz-opt" type="button" data-o="' + oi + '">' + esc(t(o)) + "</button>";
              }).join("");
              return '<li class="quiz-item" data-item data-q="' + qi + '">' +
                '<p class="quiz-q"><span class="quiz-num">' + (qi + 1) + "</span>" + esc(t(q.q)) + "</p>" +
                '<div class="quiz-opts">' + opts + "</div>" +
                '<p class="quiz-explain" hidden></p></li>';
            }).join("") + "</ol>";
          var scoreEl = document.getElementById("quizScore");
          [].forEach.call(view.querySelectorAll(".quiz-item"), function (item) {
            var qi = parseInt(item.dataset.q, 10);
            var q = qs[qi];
            ...
            opts.forEach(function (btn) {
              btn.addEventListener("click", function () {
                ...
                opts.forEach(function (b, bi) {
                  ...
                  else if (bi === chosen) b.classList.add("quiz-opt--wrong");
                });
                explain.textContent = (isRight ? "✅ " : "❌ ") + t(q.explain);
                explain.hidden = false;
                scoreEl.textContent = scoreLabel + ": " + correct + " / " + qs.length;
              });
            });
          });
        }

        var views = { glossary: glossaryView, flashcards: flashView, quiz: quizView };
        function activate(tab) {
          segBtns.forEach(function (b) { b.classList.toggle("seg__btn--active", b.dataset.tab === tab); });
          (views[tab] || glossaryView)();
        }
