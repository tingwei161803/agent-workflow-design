...
> 直接點進去就能用，無需安裝。每個子頁都有獨立網址（例如 `…/chaining.html`、`…/agent.html`），方便分享到特定模式。中文在根目錄，英文在 `/en/` 底下同名的那一頁（例如 `…/en/chaining.html`）。

---

## ✨ 功能特色

- 🌏 **中英雙語，各有網址** — 中文在根目錄、英文在 `/en/`，切換鈕是真的連結；分享出去的網址是哪種語言，對方看到的就是哪種語言
